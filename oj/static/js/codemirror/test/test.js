...
testCM("clickFold", function (cm) { // Issue #5392
  ...
  var widget = document.createElement("span")
  widget.textContent = "<>"
  cm.markText(Pos(0, 5), Pos(0, 10), {
    replacedWith: widget
  })
  var after = cm.charCoords(Pos(0, 10))
  var foundOn = cm.coordsChar({
    left: after.left - 1,
    top: after.top + 4
  })
...
testCM("measureWrappedBidiLevel2", function (cm) {
  ...
  value: "foobar إإ إإ إإ إإ 555 بببببب",
  lineWrapping: true
})
...
testCM("lineWidgetIssue5486", function (cm) {
  ...
  var measure_1 = Math.round(cm.charCoords({
    line: 2,
    ch: 0
  }).top)
  ...
  var measure_2 = Math.round(cm.charCoords({
    line: 2,
    ch: 0
  }).top)
  ...
  var measure_3 = Math.round(cm.charCoords({
    line: 2,
    ch: 0
  }).top)
...
function mouseDown(cm, button, pos, mods) {
  ...
  if (mods)
    for (var prop in mods) event[prop] = mods[prop]
...
testCM("rtl_wrapped_selection", function (cm) {
  ...
  value: new Array(10).join(" فتي تم تضمينها فتي تم"),
  lineWrapping: true
})
